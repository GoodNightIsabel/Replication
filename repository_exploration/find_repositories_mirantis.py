...
# Repo link storage
mirantis_path = '../replication/mirantis_repos.txt'

# Percentage storage
title = ['Repo_url', 'Puppet_percentage']
mirantis_percentage = './mirantis_repos.csv'
...
for repo in mirantis_repositories:
    ...
    flag = True
    if not os.path.exists('../replication/repositories/Mirantis/%s'%repo.split('/')[-1]):
        Repo.clone_from(repo, '../replication/repositories/Mirantis/%s'%repo.split('/')[-1])
        
    r = Repo('../replication/repositories/Mirantis/%s'%repo.split('/')[-1])
