...
# Repo link storage
opendev_path = './LOG6307E/replication/opendev_repos.txt'

# Percentage storage
title = ['Repo_url', 'Puppet_percentage']
opendev_percentage = './LOG6307E/replication/opendev_repos.csv'
...
for repo in opendev_repositories:
    ...
    flag = True
    if not os.path.exists('./LOG6307E/replication/repositories/Opendev/%s'%repo.split('/')[-1]):
        Repo.clone_from(repo, './LOG6307E/replication/repositories/Opendev/%s'%repo.split('/')[-1])
        
    r = Repo('./LOG6307E/replication/repositories/Opendev/%s'%repo.split('/')[-1])
